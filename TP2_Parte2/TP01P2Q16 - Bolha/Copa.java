...
public class Copa {
	public static void main (String[] args) {
		...
		Arq.openWrite("matrı́cula bolha.txt");
...
class Ranking extends Time {
	...
	public static int procurar() {
		int comparacoes = 0;
        	for (int i = (c - 1); i > 0; i--) {
            		for (int j = 0; j < i; j++) {
                		if (team[j].getSG() > team[j + 1].getSG()) {
               				swap(j, j+1);
					comparacoes++;
                		}
            		}
        	}
		comparacoes += ordena();//caso o SG for igual os times serao organizados em ordem alfabetica
		return comparacoes;
   	}
