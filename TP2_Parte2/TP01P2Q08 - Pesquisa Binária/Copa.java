...
public class Copa {
	public static void main (String[] args) {
		...
		for (String line = MyIO.readLine();line.equals("FIM") == false;line = MyIO.readLine()) {
			comparacoes += rk.procurar(line);
		}
		long fim = System.currentTimeMillis();

		Arq.openWrite("matrı́cula binaria.txt");
...
class Ranking extends Time {
	...
	public static int procurar(String s) {
		int comparacoes = ordenaVetor();
		boolean leituraValida = false;
		int esq = 0 ,dir = c;
		int pos = 0;
	
		while(!leituraValida && esq < dir) {
		
			pos = (esq+dir)/2;
			if(s.equals(team[pos].getPais())) {
				leituraValida = true;				
				comparacoes++;
			}else {
				if(s.compareTo(team[pos].getPais()) > 0) {//caso a string do parametro for "maior" estara do lado direito
					esq = pos+1;
					comparacoes++;
				}else {
		...
		if(leituraValida) {//boolean verdadeiro escreva sim caso contrario nao
			 MyIO.println(s+" -- SIM");
		}else {
			 MyIO.println(s+" -- NÃO");
		}//fim else
				...
				if(time2.charAt(0) > time1.charAt(0)) {
					...
					if(time2.charAt(1) > time1.charAt(1)) {
						...
						if(time2.charAt(2) > time1.charAt(2)) {
							...
							team[j] = aux;
							comparacoes++; 
						}else if(time2.charAt(2) == time1.charAt(2)) {
							if(time2.charAt(3) > time1.charAt(3)) {
								aux = team[i];//.clone();
								team[i] = team[j];
								team[j] = aux;
								comparacoes++;
							}else if(time2.charAt(3) == time1.charAt(3)) {
								if(time2.charAt(4) > time1.charAt(4)) {
									aux = team[i];//.clone();
									team[i] = team[j];
									team[j] = aux;
									comparacoes++;
								}else if(time2.charAt(4) == time1.charAt(4)) {
									if(time2.charAt(5) > time1.charAt(5)) {
										aux = team[i];//.clone();
										team[i] = team[j];
										team[j] = aux;
										comparacoes++;
