...
public class Copa {
	public static void main (String[] args) {
		...
		Arq.openWrite("matrı́cula shellsort.txt");
...
class Ranking extends Time {
	...
	public static int procurar() {
		...
		comparacoes += ordena();//ordenacao para caso haja paises com empates iguais
		return comparacoes;
   	}//fim procurar(shellsort)
 
   public static int insercaoPorCor(int cor, int h){
	Time t = new Time();
	int comparacoes = 0;
      for (int i = (h + cor); i < c; i+=h) {
         int tmp = team[i].getDraws();
	 t = team[i];
         int j = i - h;
         while ((j >= 0) && (team[j].getDraws() > tmp)) {
            team[j + h] = team[j];
            j-=h;
	    comparacoes++;
         }//fim while
	...
	public static int ordena() {
		...
		for(int i = 0;i < c;i++) {
			for(int j = 0;j < c;j++) {
			    ...
			    if(team[i].getDraws() == team[j].getDraws()){
