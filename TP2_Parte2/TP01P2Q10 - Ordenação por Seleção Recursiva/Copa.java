...
public class Copa {
	public static void main (String[] args) {
		...
		Arq.openWrite(" matrícula_selecaoRecursiva.txt.");
...
class Ranking extends Time {
	...
	public int procurar() {
        int comparacoes = procurar(0,0);
		comparacoes += ordena();
		return comparacoes;
    }

	public int procurar(int i,int comp) {
        if (i < c - 1) {
            int menor = i;
            for (int j = i + 1; j < c; j++) {
                if (team[j].getPontos() < team[menor].getPontos()) {
                    menor = j;
					comp++;
                }
            }
            swap(i, menor); //trocar os elementos
            procurar(++i,comp);
        }
		return comp;
   }
