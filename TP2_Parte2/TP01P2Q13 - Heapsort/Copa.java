...
class Ranking extends Time {
	...
	public static void constroi(int tamHeap){
      for(int i = tamHeap; i > 1 && team[i].getLoses() > team[i/2].getLoses(); i /= 2){
         swap(i, i/2);
      }
   }
 
  public static int reconstroi(int tamHeap){
      int i = 1, filho,comparacoes=0;
      while(i <= (tamHeap/2)){
 
         if (team[2*i].getLoses() > team[2*i+1].getLoses() || 2*i == tamHeap){
            filho = 2*i;	
			comparacoes++;
         } else {
            filho = 2*i + 1;
         ...
         if(team[i].getLoses() < team[filho].getLoses()){
            swap(i, filho);
            i = filho;
			comparacoes++;
         }else{
    ...
    public static int procurar() {
      ...
      while(tamHeap > 1){
         swap(1, tamHeap--);
         comparacoes += reconstroi(tamHeap);
      }
 		comparacoes += ordena();
	...
	public static int ordena() {
		...
		for(int i = 1;i < c;i++) {
			for(int j = 1;j < c;j++) {
			    ...
			    if(team[i].getLoses() == team[j].getLoses()){
