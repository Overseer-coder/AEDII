...
public class Copa {
	public static void main (String[] args) {
		...
		Arq.openWrite("matrı́cula countingsort.txt");
...
class Ranking extends Time {
	...
	public static int getMaior() {
       int maior = team[0].getGC();
 
        for (int i = 0; i < n; i++) {
         if(maior < team[i].getGC()){
            maior = team[i].getGC();
         }
        }
       return maior;    
    ...
    public static int procurar() {
      ...
      Time[] ordenado = new Time[c];
 
      //Inicializar cada posicao do array de contagem 
        for (int i = 0; i < count.length; count[i] = 0, i++);
 
      //Agora, o count[i] contem o numero de elemento iguais a i
      for (int i = 0; i < c; count[team[i].getGC()]++, i++);
 
      //Agora, o count[i] contem o numero de elemento menores ou iguais a i
      for(int i = 1; i < count.length; count[i] += count[i-1], i++);
 
      //Ordenando
      for(int i = c-1; i >= 0; ordenado[count[team[i].getGC()]-1] = team[i], count[team[i].getGC()]--, i--);
 
      //Copiando para o array original
      for(int i = 0; i < c; team[i] = ordenado[i], i++);

		int comparacoes = ordena();
		return comparacoes;
   }
