...
public class Copa {
	public static void main (String[] args) {
		...
		Arq.openWrite("matrı́cula insercao.txt");
	...
	public static int procurar() {
		...
		Time tmp = new Time();
        	for (int i = 1; i < c; i++) {
         		...
         		while ((j >= 0) && (team[j].getWins() > tmp.getWins())) {
            			team[j + 1] = team[j];
            			j--;
         		}
         		team[j + 1] = tmp;
		...
		comparacoes += ordena();//caso as vitorias sejam iguais serao ordenados em ordem alfabetica	
	...
	public static int ordena() {
		...
		for(int i = 0;i < c;i++) {
			for(int j = 0;j < c;j++) {
			    ...
			    if(team[i].getWins() == team[j].getWins()){
