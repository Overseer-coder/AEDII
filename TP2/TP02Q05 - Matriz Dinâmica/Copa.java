...
public class Copa {
	public static void main (String[] args) {
		...
		Arq.openWrite("matrı́cula quicksort.txt");
...
class Ranking extends Time {
	...
	public static int procurar() {
      		int comparacoes = procurar(0, n-1);
		comparacoes += ordena();//ordenar em ordem alfabetica paises com o mesmo numero de gols pro
		return comparacoes;
   	}
 
 
    private static int procurar(int esq, int dir) {
        int i = esq, j = dir;
        int pivo = team[(dir+esq)/2].getGP();
	int comparacoes = 0;
        while (i <= j) {
            while (team[i].getGP() < pivo) i++;
            while (team[j].getGP() > pivo) j--;
            if (i <= j) {
                swap(i, j);
                i++;
                j--;
        ...
        if (esq < j)  comparacoes += 1 + procurar(esq, j);
        if (i < dir)  comparacoes += 1 + procurar(i, dir);
	return comparacoes;
    }
